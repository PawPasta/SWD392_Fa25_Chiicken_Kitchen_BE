...
class DailyMenuController(
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Auto-generate daily menus (Admin only)")
    @PostMapping("/auto-generate")
    fun autoGenerate(@RequestBody(required = false) req: AutoGenerateDailyMenuRequest?): ResponseEntity<ResponseModel> {
        val created = dailyMenuService.autoGenerate(req ?: AutoGenerateDailyMenuRequest())
        return ResponseEntity.ok(ResponseModel.success(created, "Auto-generated ${created.size} daily menu(s) successfully!"))
    }
